...
interface SearchBody { results: Array<{ id: string; score: number }>; error?: string }
interface StatsBody { total_docs: number; total_vocab: number; pagerank_ready: boolean; live_nodes: number }
...
describe("GET /search", () => {
  ...
  test("fan-out finds doc indexed on mock node", async () => {
    ...
    const res = await app.handle(new Request("http://localhost/search?q=rust&limit=5"))
    expect(res.status).toBe(200)
  ...
  test("fan-out across two nodes merges results via RRF", async () => {
    ...
    const res = await app.handle(new Request("http://localhost/search?q=async+rust&limit=10"))
    expect(res.status).toBe(200)
    const body = await asJson<SearchBody>(res)
  ...
  test("dead node is skipped gracefully — other shards still return results", async () => {
    ...
    await app.handle(
      new Request("http://localhost/nodes/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nodeId: "cc".repeat(32), url: "http://localhost:19099" }),
      }),
    )

    const res = await app.handle(new Request("http://localhost/search?q=search&limit=5"))
    expect(res.status).toBe(200)
    const body = await asJson<SearchBody>(res)
